...
export default (function () {
  ...
  function init(list) {
    ...
    const listActions = list.querySelectorAll(".list-muti-actions .action");
    ...
    if (listCheckboxAll) {
      listCheckboxAll.addEventListener("change", function () {
        ...
        countCheckboxes(listCheckboxes, listAlert, listAlertCount, listActions);
      });
    }
  ...
  function countCheckboxes(
    ...
    if (listAlert) {
      checked.length
        ? listAlert.classList.add("show")
        : listAlert.classList.remove("show");
      listAlertCount.innerHTML = checked.length;
    }

    const selectItems = checked.map((item) => JSON.parse(item.dataset.item));
    if (selectItems.length > 0) {
      [].forEach.call(listActions, function (action) {
        if (checked && checked.length > 0) {
          action.dataset.item = JSON.stringify(selectItems);
        }
      });
    } else {
      [].forEach.call(listActions, function (action) {
        delete action.dataset.item;
      });
    }
  }
